...
// Libmorton - Methods to encode/decode 64-bit morton codes from/to 32-bit (x,y)
// coordinates Warning: morton.h will always point to the functions that use the
...
template <typename morton, typename coord>
__device__ __host__ inline void m2D_d_sLUT_ET(const morton m, coord& x,
                                              coord& y);
template <typename morton, typename coord>
__device__ __host__ inline void m2D_d_LUT(const morton m, coord& x, coord& y);
template <typename morton, typename coord>
__device__ __host__ inline void m2D_d_LUT_ET(const morton m, coord& x,
                                             coord& y);
template <typename morton, typename coord>
__device__ __host__ inline void m2D_d_magicbits(const morton m, coord& x,
                                                coord& y);
template <typename morton, typename coord>
...
template <typename morton, typename coord>
__device__ __host__ inline morton compute2D_ET_LUT_encode(const coord c,
                                                          const coord* LUT) {
...
__device__ __host__ inline morton m2D_e_sLUT_ET(const coord x, const coord y) {
  morton answer_x =
      compute2D_ET_LUT_encode<morton, coord>(x, Morton2D_encode_x_256);
  morton answer_y =
      compute2D_ET_LUT_encode<morton, coord>(y, Morton2D_encode_y_256);
...
__device__ __host__ inline morton m2D_e_LUT_ET(const coord x, const coord y) {
  morton answer_x =
      compute2D_ET_LUT_encode<morton, coord>(x, Morton2D_encode_x_256);
  morton answer_y =
      compute2D_ET_LUT_encode<morton, coord>(y, Morton2D_encode_x_256);
  return (answer_y << 1) | answer_x;
}

// Magicbits masks (2D encode)
static uint_fast32_t magicbit2D_masks32[6] = {
    0xFFFFFFFF, 0x0000FFFF, 0x00FF00FF, 0x0F0F0F0F, 0x33333333, 0x55555555};
static uint_fast64_t magicbit2D_masks64[6] = {
...
__device__ __host__ inline morton morton2D_SplitBy2Bits(const coord a) {
  const morton* masks =
      (sizeof(morton) <= 4)
          ? reinterpret_cast<const morton*>(magicbit2D_masks32)
...
template <typename morton, typename coord>
__device__ __host__ inline morton m2D_e_magicbits(const coord x,
                                                  const coord y) {
...
// https://gist.github.com/JarkkoPFC/0e4e599320b0cc7ea92df45fb416d79a This uses
// the same technique as the magicbits method, but uses the upper part of a
// 64-bit type to split the y coordinate, the lower part to split the x
// coordinate, then merges them back together.
__device__ __host__ inline uint_fast32_t m2D_e_magicbits_combined(
    uint_fast16_t x, uint_fast16_t y) {
  uint_fast64_t m = x | (uint_fast64_t(y)
  ...
  // hard cut off to 32 bits, because on some systems uint_fast32_t will be a
  // 64-bit type, and we don't want to retain split Y-version in the upper 32
  // bits.
...
__device__ __host__ inline morton m2D_e_for_ET(const coord x, const coord y) {
  ...
  checkbits = std::min(static_cast<unsigned long>(checkbits),
                       std::max(x_max, y_max) + 1ul);
  for (unsigned int i = 0; i < checkbits; ++i) {
    morton m_shifted =
        static_cast<morton>(0x1)
        << i;  // Here we need to cast 0x1 to 64bits, otherwise there is a bug
...
template <typename morton, typename coord>
__device__ __host__ inline coord morton2D_DecodeCoord_LUT256(
...
__device__ __host__ inline void m2D_d_LUT_ET(const morton m, coord& x,
  ...
  while (firstbit_location > i) {
    ...
    y |= (coord)Morton2D_decode_x_256[(m >> (i + 1)) & EIGHTBITMASK]
         << shiftback;
    shiftback += 4;
    i += 8;
  }
...
__device__ __host__ static inline coord morton2D_GetSecondBits(const morton m) {
  morton* masks = (sizeof(morton) <= 4)
                      ? reinterpret_cast<morton*>(magicbit2D_masks32)
...
__device__ __host__ inline void m2D_d_magicbits_combined(
    const uint_fast32_t morton, uint_fast16_t& x, uint_fast16_t& y) {
  uint_fast64_t res =
      (morton | (uint_fast64_t(morton) << 31)) & magicbit2D_masks64[5];
...
template <typename morton, typename coord>
__device__ __host__ inline void m2D_d_for_ET(const morton m, coord& x,
                                             coord& y) {
  x = 0;
  y = 0;
  unsigned long firstbit_location = 0;
  ...
  unsigned int checkbits = static_cast<unsigned int>(
      std::min(defaultbits, firstbit_location / 2.0f));
  for (unsigned int i = 0; i <= checkbits; ++i) {
    morton selector = 1;
    unsigned int shift_selector = 2 * i;
    x |= (m & (selector << shift_selector)) >> i;
