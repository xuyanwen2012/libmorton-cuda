...
// Libmorton - Methods to encode/decode 64-bit morton codes from/to 32-bit
// (x,y,z) coordinates Warning: morton.h will always point to the functions that
...
__device__ __host__ inline morton m3D_e_sLUT(const coord x, const coord y,
                                             const coord z);
template <typename morton, typename coord>
__device__ __host__ inline morton m3D_e_sLUT_ET(const coord x, const coord y,
                                                const coord z);
template <typename morton, typename coord>
__device__ __host__ inline morton m3D_e_LUT(const coord x, const coord y,
                                            const coord z);
template <typename morton, typename coord>
__device__ __host__ inline morton m3D_e_LUT_ET(const coord x, const coord y,
                                               const coord z);
template <typename morton, typename coord>
__device__ __host__ inline morton m3D_e_magicbits(const coord x, const coord y,
                                                  const coord z);
template <typename morton, typename coord>
__device__ __host__ inline morton m3D_e_for(const coord x, const coord y,
                                            const coord z);
template <typename morton, typename coord>
__device__ __host__ inline morton m3D_e_for_ET(const coord x, const coord y,
                                               const coord z);

// AVAILABLE METHODS FOR DECODING
template <typename morton, typename coord>
__device__ __host__ inline void m3D_d_sLUT(const morton m, coord& x, coord& y,
                                           coord& z);
template <typename morton, typename coord>
__device__ __host__ inline void m3D_d_sLUT_ET(const morton m, coord& x,
                                              coord& y, coord& z);
template <typename morton, typename coord>
__device__ __host__ inline void m3D_d_LUT(const morton m, coord& x, coord& y,
                                          coord& z);
template <typename morton, typename coord>
__device__ __host__ inline void m3D_d_LUT_ET(const morton m, coord& x, coord& y,
                                             coord& z);
template <typename morton, typename coord>
__device__ __host__ inline void m3D_d_magicbits(const morton m, coord& x,
                                                coord& y, coord& z);
template <typename morton, typename coord>
__device__ __host__ inline void m3D_d_for(const morton m, coord& x, coord& y,
                                          coord& z);
template <typename morton, typename coord>
__device__ __host__ inline void m3D_d_for_ET(const morton m, coord& x, coord& y,
                                             coord& z);
...
__device__ __host__ inline morton m3D_e_sLUT(const coord x, const coord y,
  ...
  for (unsigned int i = sizeof(coord); i > 0; --i) {
    ...
    answer =
        answer << 24 | (Morton3D_encode_z_256[(z >> shift) & EIGHTBITMASK] |
...
template <typename morton, typename coord>
__device__ __host__ inline morton compute3D_ET_LUT_encode(const coord c,
                                                          const coord* LUT) {
...
// ENCODE 3D Morton code : Pre-shifted LookUpTable (LUT) (Early Termination
// version) This version tries to terminate early when there are no more bits to
// process Figuring this out is probably too costly in most cases.
template <typename morton, typename coord>
__device__ __host__ inline morton m3D_e_sLUT_ET(const coord x, const coord y,
                                                const coord z) {
  morton answer_x =
      compute3D_ET_LUT_encode<morton, coord>(x, Morton3D_encode_x_256);
  morton answer_y =
      compute3D_ET_LUT_encode<morton, coord>(y, Morton3D_encode_y_256);
  morton answer_z =
      compute3D_ET_LUT_encode<morton, coord>(z, Morton3D_encode_z_256);
...
__device__ __host__ inline morton m3D_e_LUT_ET(const coord x, const coord y,
  ...
  morton answer_x =
      compute3D_ET_LUT_encode<morton, coord>(x, Morton3D_encode_x_256);
  morton answer_y =
      compute3D_ET_LUT_encode<morton, coord>(y, Morton3D_encode_x_256);
  morton answer_z =
      compute3D_ET_LUT_encode<morton, coord>(z, Morton3D_encode_x_256);
...
// This method uses certain bit patterns (magic bits) to split bits in the
// coordinates
template <typename morton, typename coord>
__device__ __host__ inline morton m3D_e_magicbits(const coord x, const coord y,
                                                  const coord z) {
...
__device__ __host__ inline morton m3D_e_for_ET(const coord x, const coord y,
  ...
  checkbits =
      std::min((unsigned long)checkbits,
               std::max(z_max, std::max(x_max, y_max)) + (unsigned long)1);
  for (unsigned int i = 0; i < checkbits; ++i) {
    morton m_shifted =
        static_cast<morton>(1)
        << i;  // Here we need to cast 0x1 to 64bits, otherwise there is a bug
...
__device__ __host__ inline void m3D_d_LUT_ET(const morton m, coord& x, coord& y,
  ...
  while (i < firstbit_location) {
    ...
    y = y | (coord)Morton3D_decode_x_512[(m >> (i + 1)) & NINEBITMASK]
                << shiftback;
    z = z | (coord)Morton3D_decode_x_512[(m >> (i + 2)) & NINEBITMASK]
                << shiftback;
    i += 9;
    shiftback += 3;
  }
...
// This method splits the morton codes bits by using certain patterns (magic
// bits)
template <typename morton, typename coord>
__device__ __host__ inline void m3D_d_magicbits(const morton m, coord& x,
                                                coord& y, coord& z) {
...
__device__ __host__ inline void m3D_d_for_ET(const morton m, coord& x, coord& y,
  ...
  unsigned int checkbits = static_cast<unsigned int>(
      std::min((float)defaultbits, firstbit_location / 3.0f));
  for (unsigned int i = 0; i <= checkbits; ++i) {
    morton selector = 1;
    unsigned int shift_selector = 3 * i;
    unsigned int shiftback = 2 * i;
