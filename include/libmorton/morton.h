...
// This file will always contain inline functions which point to the fastest
// Morton encoding/decoding implementation IF you just want to use the fastest
// method to encode/decode morton codes, include this header.

// If you want to experiment with alternative methods (which might be slower,
// all depending on hardware / your data set) check the individual headers
// below.
...
namespace libmorton {
// Functions under this are stubs which will always point to fastest
// implementation at the moment
//-----------------------------------------------------------------------------------------------

// ENCODING
#if defined(__BMI2__) || defined(__AVX2__)
inline uint_fast32_t morton2D_32_encode(const uint_fast16_t x,
                                        const uint_fast16_t y) {
  return m2D_e_BMI<uint_fast32_t, uint_fast16_t>(x, y);
}
inline uint_fast64_t morton2D_64_encode(const uint_fast32_t x,
                                        const uint_fast32_t y) {
  return m2D_e_BMI<uint_fast64_t, uint_fast32_t>(x, y);
}
inline uint_fast32_t morton3D_32_encode(const uint_fast16_t x,
                                        const uint_fast16_t y,
                                        const uint_fast16_t z) {
  return m3D_e_BMI<uint_fast32_t, uint_fast16_t>(x, y, z);
}
inline uint_fast64_t morton3D_64_encode(const uint_fast32_t x,
                                        const uint_fast32_t y,
...
#else
__device__ __host__ inline uint_fast32_t morton2D_32_encode(
    const uint_fast16_t x, const uint_fast16_t y) {
  return m2D_e_magicbits_combined(x, y);
}
__device__ __host__ inline uint_fast64_t morton2D_64_encode(
    const uint_fast32_t x, const uint_fast32_t y) {
  return m2D_e_sLUT<uint_fast64_t, uint_fast32_t>(x, y);
}
__device__ __host__ inline uint_fast32_t morton3D_32_encode(
    const uint_fast16_t x, const uint_fast16_t y, const uint_fast16_t z) {
  return m3D_e_sLUT<uint_fast32_t, uint_fast16_t>(x, y, z);
}
__device__ __host__ inline uint_fast64_t morton3D_64_encode(
    const uint_fast32_t x, const uint_fast32_t y, const uint_fast32_t z) {
